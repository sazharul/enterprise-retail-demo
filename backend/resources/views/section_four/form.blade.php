...
<div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
    ...
    <select class="form-control" name="status">
        <option value="1" {{ isset($sections->status) && $sections->status == 1 ? 'selected' : '' }}>Active</option>
        <option value="0" {{ isset($sections->status) && $sections->status == 0 ? 'selected' : '' }}>InActive</option>
    </select>
