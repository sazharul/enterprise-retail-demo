...
use App\Models\SectionThirteen;
use Illuminate\Http\Request;
use RealRashid\SweetAlert\Facades\Alert;

class SectionThirteenController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($keyword)) {
            $sections = SectionThirteen::where('name', 'LIKE', "%$keyword%")
                ->orWhere('status', 'LIKE', "%$keyword%")
                ->orderBy('id', 'asc')->paginate($perPage);
        } else {
            $sections = SectionThirteen::orderBy('id', 'asc')->paginate($perPage);
        }

        return view('section_thirteen.index', compact('sections'));
    }
    ...
    public function create(Request $request)
    {
        ...
        $sections = SectionThirteen::where('status', 1)->get();

        return view('section_thirteen.create', compact('offers', 'sections'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // dd($checkedData);
        SectionThirteen::truncate();
        foreach ($checkedData as $data) {

            SectionThirteen::updateOrCreate(
                [
        ...
        Alert::toast('Section 13 data updated successfully', 'success');

        return redirect()->route('section_thirteen.index');
    }
